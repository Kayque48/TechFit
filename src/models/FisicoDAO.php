...
class FisicoDAO {
    ...
    public function buscarPorIdAluno($idAluno) {
        $stmt = $this->conn->prepare("SELECT * FROM AVALIACOES_FISICAS WHERE FK_ALUNO = :idAluno ORDER BY DATA_AVALIACAO DESC");
    ...
    public function buscarPorDataEAluno($data, $idAluno) {
        $stmt = $this->conn->prepare("SELECT * FROM AVALIACOES_FISICAS WHERE DATA_AVALIACAO = :data AND FK_ALUNO = :idAluno LIMIT 1");
        $stmt->execute([':data' => $data, ':idAluno' => $idAluno]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            return new Fisico(
                $row['DATA_AVALIACAO'],
